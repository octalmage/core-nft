...
func (k Keeper) getContractDetailsQuery(ctx sdk.Context, contractAddress sdk.AccAddress) (codeInfo types.CodeInfo, contractStorePrefix prefix.Store, err error) {
	store := ctx.KVStore(k.storeKey)
	...
	if correctedCodeId, ok := codeCorrections[contractInfo.CodeID]; ok {
		bz = store.Get(types.GetCodeInfoKey(codeCorrections[correctedCodeId]))
	} else {
		bz = store.Get(types.GetCodeInfoKey(contractInfo.CodeID))
	}
